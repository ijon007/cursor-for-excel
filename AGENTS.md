...
### Project Overview
This is **Cursor for Excel** — a Next.js 16 app with a split-pane interface: Univer spreadsheet (left) + AI chat panel (right). Uses Bun as package manager.
...
- Univer is initialized in a client-only component (`components/spreadsheet.tsx`) using `createUniver()` and `UniverSheetsCorePreset` with a DOM container.
- The active workbook facade API is stored in `lib/store.ts` (`getWorkbookApi()`). Tool calls in `lib/tools.ts` use this to manipulate cells programmatically.
...
- Tool calls execute on the server (with `execute` functions) and are mirrored on the client via `executeToolOnClient()` in `lib/tools.ts` which maps tool names to Univer facade API calls.
...
- **Univer CSS**: Imported inside client component `components/spreadsheet.tsx`. Cannot be imported from a Server Component.
